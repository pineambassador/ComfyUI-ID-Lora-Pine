...
class IDLoRASeparateLatent:
    ...
    def separate(self, combined_latent):
        ...
        # 2. Audio reshaped to 4D [B, C, F, (H*W)]
        ...
        # 3. Handle Mask (The 'Safe' Version)
        if "noise_mask" in combined_latent:
            mask = combined_latent["noise_mask"]
            if mask is not None:
                if torch.is_tensor(mask):
                    # Just give the first part of the batch to Video
                    # We use [0:1] to keep it as a batch of 1
                    video_out["noise_mask"] = mask[0:1, ...]
                    
                    # WE IGNORE AUDIO MASK HERE
                    # This prevents the "too many values to unpack" error 
                    # at your line 449 and avoids confusing the Audio VAE.
                    
                elif isinstance(mask, (list, tuple)) and len(mask) >= 2:
                    video_out["noise_mask"] = mask[0]

        print(f"DEBUG [Splitter]: Video Latent Shape: {video_samples.shape}")
        print(f"DEBUG [Splitter]: Audio Latent Shape: {audio_samples.shape}")
